...
private const val PKG_NIEHE = "com.soreverse.mcp" // SOMCP（聚合逆向后端，包名 com.soreverse.mcp）
...
fun ReverseWorkbenchCard(
    ...
    // 挨个探测各后端的常见端口，哪个通就把对应 MCP 的地址改成那个，实现一键连接。
    ...
    Card(
        ...
        Column(
            ...
            Text(
                text = "预置 3 个本机逆向 MCP 后端（SOMCP 已聚合全套逆向工具），每个可单独开关、单独改端口。" +
            ...
            FlowRow(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                OutlinedButton(onClick = {
                    ...
                    Text(if (context.isPackageInstalled(PKG_NIEHE)) "打开 SOMCP" else "装 SOMCP")
                }
                OutlinedButton(onClick = {
            ...
            Text(
                text = "提示：启用后请到对应 App 里启动服务——MT 管理器在侧边栏开启「APK MCP」并保持后台，" +
                    "SOMCP 在首页点大启动按钮开服务（端口 8000）。都监听 127.0.0.1，仅本机可用。",

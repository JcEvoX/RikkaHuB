...
class SettingsStore(
    ...
    val settingsFlowRaw = dataStore.data
        ...
        .map {
            ...
            // RikkaHub：预置 MT 管理器 / SOMCP / ProxyPin 逆向 MCP 后端（默认禁用，不产生任何连接）。
            // 必须在下方"清理无效 MCP 引用"之前注入，否则逆向工作台助手对它们的引用会被过滤掉。
            val mcpServers = it.mcpServers.toMutableList()
            // 迁移：移除已废弃的"算法助手"预置后端（SOMCP 已聚合其能力）。老用户升级时清掉这条僵尸项。
            ...
            // RikkaHub：为老用户补齐逆向任务模板（预置快捷消息），不存在才加，保留用户自己建的。
...
data class Settings(
    ...
    // RikkaHub：自动压缩会话默认关。阈值20万token触发，压前保留最近5条原文，摘要目标4000token，短会话(<20条)不压。用户可在设置调。
    ...
    // RikkaHub：默认助手设为「逆向工作台」，一进 App 就是主打的逆向助手
    ...
    // RikkaHub：首启引导完成标记（false=第一次打开，显示引导）
...
// RikkaHub：逆向工作台的一键任务模板（预置快捷消息）。固定 UUID，点一下把标准指令填进输入框直接发。
// 这是"超越对手"的差异化——别人手动一步步操作，RikkaHub 一句话让 AI 自动干。

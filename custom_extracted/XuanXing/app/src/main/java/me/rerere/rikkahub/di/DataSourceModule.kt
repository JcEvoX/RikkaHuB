        ...
        OkHttpClient.Builder()
            ...
            .addInterceptor { chain ->
                ...
                if (originalRequest.header(HttpHeaders.UserAgent) == null) {
                    // HTTP header 值只允许 ASCII。versionName 若含非 ASCII（如中文魔改名）
                    // 会让 okhttp 抛 IllegalArgumentException 直接崩溃，这里过滤兜底。
                    val safeVersion = BuildConfig.VERSION_NAME.filter { it.code in 0x20..0x7E }
                        .ifBlank { BuildConfig.VERSION_CODE.toString() }
                    requestBuilder.addHeader(HttpHeaders.UserAgent, "RikkaHub-Android/$safeVersion")
                }
